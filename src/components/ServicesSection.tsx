import {
  Fish, Waves, Palmtree, Package, Sparkles, Clock,
  Anchor, Ship, Star, Compass, Plus, ArrowRight, CheckCircle2,
  LucideIcon,
} from "lucide-react";
import { useState, useEffect } from "react";
import DecorativeBackground from "./DecorativeBackground";
import { servicesAPI, contactAPI } from "@/lib/api";

// Map nama icon (string dari DB) → komponen Lucide
const ICON_MAP: Record<string, LucideIcon> = {
  Fish, Waves, Palmtree, Package, Sparkles, Clock,
  Anchor, Ship, Star, Compass,
};
...
const ServicesSection = () => {
  ...
  const [phoneNumber, setPhone]       = useState("6281534475202");
    ...
    const fetchData = async () => {
      try {
        ...
        if (contactRes.data?.whatsapp_number) {
          setPhone(contactRes.data.whatsapp_number);
        }
      } catch (err) {
  ...
  return (
    <section id="layanan" className="py-20 bg-background relative overflow-hidden">
        ...
        {!loading && (
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto">
            {services.map((service) => {
              const IconComponent = ICON_MAP[service.icon_name] ?? Star;
              ...
              return (
                ...
                >
                  ...
                  >
                    <div className="p-6 flex flex-col min-h-[320px]">
                      ...
                      <div className={`
                        ...
                        ${service.is_highlight
                          ...
                          : "bg-gradient-to-br from-primary to-[hsl(210_100%_60%)]"
                        }
                      `}>
                      ...
                      {/* Title */}
                      <h3 className={`text-xl font-semibold mb-3 ${service.is_highlight ? "text-primary-foreground" : "text-foreground"}`}>
                        {service.title}
                      </h3>
                      ...
                      <div className="relative flex-1 mb-4">
                        ...
                        <div className="service-description absolute inset-0">
                          <p className={`text-sm leading-relaxed ${service.is_highlight ? "text-primary-foreground/90" : "text-muted-foreground"}`}>
                            {service.description}
                          </p>
                        </div>
                        ...
                        <div className="service-features absolute inset-0">
                          <div className="space-y-2">
                            {(service.features ?? []).map((feature, idx) => (
                              ...
                              >
                                <CheckCircle2 className={`w-4 h-4 flex-shrink-0 mt-0.5 ${service.is_highlight ? "text-primary-foreground" : "text-green-500"}`} />
                                <span className={`text-sm ${service.is_highlight ? "text-primary-foreground" : "text-foreground"}`}>
                                  {feature}
                                </span>
                              </div>
                            ))}
                      ...
                      <div className="mt-auto relative h-10">
                        ...
                        <button className={`
                          ...
                          ${service.is_highlight ? "border-primary-foreground" : "border-primary"}
                        `}>
                          <Plus className={`w-5 h-5 ${service.is_highlight ? "text-primary-foreground" : "text-primary"}`} />
                        </button>
                    ...
                    {/* Top Border */}
                    <div className={`service-top-border absolute top-0 left-0 right-0 h-1 ${service.is_highlight ? "bg-primary-foreground" : "bg-primary"}`} />
                  </div>
                </div>
              );
            })}
      ...
      <style>{`
        .service-card-wrapper { will-change: transform; }
        ...
        @media (prefers-reduced-motion: reduce) {
          .service-card, .service-description, .service-features,
          .service-feature-item, .service-plus-btn, .service-cta-btn,
          .service-top-border { transition-duration: 0.01ms !important; }
        }
      `}</style>
    </section>
  );
